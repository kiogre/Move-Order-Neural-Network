"""
train_transformer_lichess.py — Training supervisionato con JellyFishPointerTransformer.
...
Differenze rispetto a train_supervised_lichess.py:
  - Modello: JellyFishPointerTransformer (transformer encoder + pointer head)
  - Optimizer: AdamW con lr separato per bias_scale (via model.get_optimizer())
  - Freeze: congela model.encoder invece di model.backbone
  - encode_board importato da pointer_dataset (restituisce (13,8,8), gestito internamente
    dall'encoder transformer)

Utilizzo:
  python train_transformer_lichess.py
...
from MLChess import encode_board, encode_legal_moves, JellyFishPointerTransformer
...
CHECKPOINT_IN    = "checkpoints_transformer/last.pt"      # parte da qui se esiste
...
def run_epoch(model, loader, optimizer, device, train=True, freeze_backbone=False):
    ...
    if freeze_backbone:
        # Congela encoder — solo move_encoder, policy_head e value_head ricevono gradiente
...
def main():
    ...
    # ---- Modello ----
    model = JellyFishPointerTransformer(
        input_dim     = 13,
        d_model       = 256,
        n_heads       = 8,
        n_layers      = 4,
        ffn_dim       = 512,
        dropout       = 0.1,
        kin_bias_path = 'attention_based_matrix_64x64.pt',
    ).to(DEVICE)
    ...
    # AdamW con lr separato per bias_scale (evita che esploda come nelle versioni MHA_*)
    optimizer = model.get_optimizer(
        lr_main  = LR_MAIN,
        lr_bias  = LR_BIAS_SCALE,
        wd       = 1e-4,
    )
    ...
    print(f"Fase 1: encoder congelato per le prime {FREEZE_EPOCHS} epoche")
    ...
    for epoch in range(start_epoch, TOTAL_EPOCHS + 1):
        ...
        if freeze and epoch == start_epoch:
            print(f"[Fase 1] Encoder congelato — LR heads: {LR_MAIN}")
        elif not freeze and (epoch == FREEZE_EPOCHS + 1 or epoch == start_epoch):
            print(f"[Fase 2] Fine-tuning completo — LR main: {LR_MAIN}, LR bias_scale: {LR_BIAS_SCALE}")
