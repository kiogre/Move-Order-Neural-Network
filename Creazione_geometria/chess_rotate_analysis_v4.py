...
sys.path.append(".")
from chess_rotate_v4 import (
    ChessRotatEv4, ChessCSVDataset,
    board_to_tensor, board_to_aux_features, move_to_features, parse_evaluation
)
...
def load_model(path, embedding_dim=128, device="cpu"):
    model = ChessRotatEv4(embedding_dim)
...
def get_embedding_vec(model, board, device):
    ...
    with torch.no_grad():
        ...
        aux = board_to_aux_features(board).unsqueeze(0).to(device)
...
def probing_analysis(model, dataset, device, n=3000):
    ...
    with torch.no_grad():
        for i in tqdm(indices, leave=False):
            pos_before, aux_before, _, _, _, ev = dataset[i]
...
def umap_analysis(embeddings, evals, dataset, indices):
    ...
    for i in indices[:len(embeddings)]:
        sample = dataset[i]
        pos_before = sample[0]  # (pos_before, aux_before, move_feat, pos_after, aux_after, eval)
...
def move_geometry_analysis(model, dataset, device, n=2000):
    ...
    with torch.no_grad():
        for i in indices:
            if len(all_angles) >= n:
                break
            pos_before, _, move_feat, _, _, _ = dataset[i]

            # Ottieni angoli — move_feat è tensore flat [16]
            feat_t = move_feat.unsqueeze(0).to(device)
            theta = model.move_encoder(feat_t).squeeze(0).cpu().numpy()

            piece_names = ["pedone", "cavallo", "alfiere", "torre", "donna", "re"]
            piece_idx_val = int(move_feat[4:10].argmax().item())
            is_capture = move_feat[10].item() > 0.5
            piece_name = piece_names[piece_idx_val]
...
if __name__ == "__main__":
    ...
    MODEL_PATH = sys.argv[2] if len(sys.argv) > 2 else "chess_rotate_v4.pt"
