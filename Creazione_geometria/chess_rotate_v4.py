...
def board_to_aux_features(board: chess.Board) -> torch.Tensor:
    """
    8 feature ausiliarie che la CNN fatica a estrarre dalla griglia:
      [0] turno (1=bianco, 0=nero)
      [1] n_pezzi_bianchi / 16
      [2] n_pezzi_neri / 16
      [3-6] diritti arrocco (KQkq)
      [7] halfmove clock / 50
    """
    feat = torch.zeros(8)
    pieces = board.piece_map()
    feat[0] = 1.0 if board.turn == chess.WHITE else 0.0
    feat[1] = sum(1 for p in pieces.values() if p.color == chess.WHITE) / 16.0
    feat[2] = sum(1 for p in pieces.values() if p.color == chess.BLACK) / 16.0
    feat[3] = float(board.has_kingside_castling_rights(chess.WHITE))
    feat[4] = float(board.has_queenside_castling_rights(chess.WHITE))
    feat[5] = float(board.has_kingside_castling_rights(chess.BLACK))
    feat[6] = float(board.has_queenside_castling_rights(chess.BLACK))
    feat[7] = board.halfmove_clock / 50.0
    return feat
...
class PositionEncoder(nn.Module):
    ...
    def forward(self, x, aux=None):
        ...
        if aux is None:
            aux = torch.zeros(flat.shape[0], 8, device=flat.device)
        combined = torch.cat([flat, aux], dim=-1)
...
class ChessRotatEv4(nn.Module):
    ...
    def forward(self, pos_before, aux_before, move_feat, pos_after, aux_after):
        h_real, h_imag = self.position_encoder(pos_before, aux_before)
        t_real, t_imag = self.position_encoder(pos_after,  aux_after)
...
def corrupt_batch(pos_after_batch, dataset, device):
    ...
    pos_neg = torch.stack([dataset[i][3] for i in indices]).to(device)
    aux_neg = torch.stack([dataset[i][4] for i in indices]).to(device)
    return pos_neg, aux_neg
...
def train(
    ...
    save_path: str = "chess_rotate_v4.pt",
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch in tqdm(loader, desc=f"Epoch {epoch:2d}/{epochs}", leave=False):
            pos_before, aux_before, move_feat, pos_after, aux_after, eval_before = batch
            pos_before  = pos_before.to(device)
            aux_before  = aux_before.to(device)
            move_feat   = move_feat.to(device)
            pos_after   = pos_after.to(device)
            aux_after   = aux_after.to(device)
            eval_before = eval_before.to(device)

            out_pos = model(pos_before, aux_before, move_feat, pos_after, aux_after)

            pos_after_neg, aux_after_neg = corrupt_batch(pos_after, dataset, device)
            out_neg = model(pos_before, aux_before, move_feat, pos_after_neg, aux_after_neg)
...
def analyze(model, csv_path, n_samples=2000,
    ...
    with torch.no_grad():
        for pos_before, aux_before, _, _, _, ev_before in tqdm(dataset, desc="Analisi"):
            t   = pos_before.unsqueeze(0).to(device)
            aux = aux_before.unsqueeze(0).to(device)
            real, imag = model.position_encoder(t, aux)
    ...
    plt.tight_layout()
    plt.savefig("analisi_v2.png", dpi=150, bbox_inches='tight')
    print("Salvato: analisi_v2.png")
    plt.close()
