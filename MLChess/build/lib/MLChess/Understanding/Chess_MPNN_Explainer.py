...
class ChessMPNNExplainer:
    ...
    def analyze_embeddings(self, data, board: chess.Board) -> np.ndarray:
        """
        Analizza l'attivazione dei nodi dopo i layer MPNN
        """
        if not hasattr(data, 'batch') or data.batch is None:
            data.batch = torch.zeros(data.x.size(0), dtype=torch.long)

        with torch.no_grad():
            x = F.relu(self.model.MPNN.input_proj(data.x))
            x = F.relu(self.model.MPNN.conv1(x, data.edge_index, data.edge_attr))
            x = F.relu(self.model.MPNN.conv2(x, data.edge_index, data.edge_attr))
            x = F.relu(self.model.MPNN.conv3(x, data.edge_index, data.edge_attr))

        activation = x.norm(dim=1)
        activation = activation / activation.max()
        heatmap = activation.cpu().numpy().reshape(8, 8)

        fig, ax = plt.subplots(1, 1, figsize=(6, 6))
        self._plot_board(board, ax)
        im = ax.imshow(heatmap[::-1], cmap='viridis', interpolation='nearest')
        ax.set_title('Node Activation (MPNN)')
        plt.colorbar(im)
        plt.tight_layout()
        return heatmap, fig
    ...
    def complete_analysis(self, data, board: chess.Board, save_path: Optional[str] = None):
        ...
        with torch.no_grad():
            x = F.relu(self.model.MPNN.input_proj(data.x))
            x = self.model.MPNN.conv1(x, data.edge_index, data.edge_attr)
            x = F.relu(x)
            x = self.model.MPNN.conv2(x, data.edge_index, data.edge_attr)
            x = F.relu(x)
            x = self.model.MPNN.conv3(x, data.edge_index, data.edge_attr)
            embeddings = F.relu(x)
        ...
        ax7.set_title('Node Activation (MPNN)', fontsize=10)
