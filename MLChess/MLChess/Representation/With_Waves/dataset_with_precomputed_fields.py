...
Fix: HDF5 opened lazily per worker — no multiprocessing conflicts.
"""
...
class ChessDatasetPrecomputed(Dataset):
    def __init__(self, csv_path, h5_path, move_vocab, indices):
    ...
    def __getitem__(self, i):
        ...
        board_planes[13] = torch.from_numpy(self.white_ds[idx].astype(np.float32))
        board_planes[14] = torch.from_numpy(self.black_ds[idx].astype(np.float32))
        board_planes[15] = torch.from_numpy(self.control_ds[idx].astype(np.float32))
...
class ChessDatasetBaseline(Dataset):
    def __init__(self, csv_path, move_vocab, indices):
        self.move_vocab = move_vocab
        self.indices    = indices
